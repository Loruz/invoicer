...
export default async function ClientsPage({
  ...
  const allClients = await getAllClients(user.id);
    ...
    <div>
      ...
      <div className="mb-8 grid grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border border-[#E8ECF1] bg-white p-5">
          ...
          <p className="mt-1 text-2xl font-bold text-slate-900">{activeProjectCount}</p>
        </div>
        <div className="rounded-xl border border-[#E8ECF1] bg-white p-5">
          <p className="text-sm text-slate-500">Total Revenue</p>
          <p className="mt-1 text-2xl font-bold text-slate-900">{formatCurrency(totalRevenue, userCurrency)}</p>
        </div>
        <div className="rounded-xl border border-[#E8ECF1] bg-white p-5">
          <p className="text-sm text-slate-500">Outstanding</p>
          <p className="mt-1 text-2xl font-bold text-amber-500">{formatCurrency(outstanding, userCurrency)}</p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#E8ECF1]">
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Projects</th>
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Total Billed</th>
            ...
            <tbody>
              {clientList.map((client) => {
                ...
                const totalBilled = client.invoices?.reduce((s, inv) => s + inv.total, 0) || 0;
                const hasActiveProjects = client.projects?.some((p) => p.isActive) ?? false;

                return (
                  <tr key={client.id} className="border-b border-[#E8ECF1] last:border-b-0 hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      <Link href={`/clients/${client.id}`} className="flex items-center gap-3">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-100 text-xs font-semibold text-blue-700">{initials}</div>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500">{projectCount} project{projectCount !== 1 ? "s" : ""}</td>
                    <td className="px-6 py-4 text-sm font-medium text-slate-900">{formatCurrency(totalBilled, userCurrency)}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${hasActiveProjects ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"}`}>
